...
export class BodyDashboardComponent implements OnInit {
  ...
  getdata(){
   ...
   this.dataService.getExamenReussi(localStorage.getItem('autoEcole_id')).subscribe(data=>{
      this.candidatReussi = JSON.parse(data);console.log(this.candidatReussi);
      console.log("examen reussi ");console.log(this.candidatReussi);
  ...
  getCandidatInscription(){
    this.dataService.getCandidats(localStorage.getItem('autoEcole_id')).subscribe(data=>{
      console.log("candidat");console.log(JSON.parse(data));
